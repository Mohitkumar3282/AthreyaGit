...
async function run() {
    try {
        ...
        // Find "Fresh Milk 1L" or any product
        const product = await Product.findOne({ name: /milk/i });
        if (!product) {
            console.error('No milk product found in DB');
            await mongoose.disconnect();
            return;
        }
        console.log('Found product:', { id: product._id, name: product.name, status: product.status, variants: product.variants });
